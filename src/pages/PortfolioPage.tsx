...
const works = [
  {
    ...
    field: "Commerce & Marketing",
    category: "management",
    level: "Master 2",
    university: "ESSEC Douala",
    slides: 22,
    gradient: "linear-gradient(135deg, #1d4ed8 0%, #0891b2 60%, #06b6d4 100%)",
    accent: "#06b6d4",
    icon: "🛒",
  ...
  },
  {
    ...
    category: "management",
    level: "Licence Pro",
    university: "Institut Supérieur de Communication",
    slides: 16,
    gradient: "linear-gradient(135deg, #ea580c 0%, #d97706 50%, #ca8a04 100%)",
    accent: "#d97706",
    icon: "📱",
  ...
  },
  {
    ...
    category: "education",
    level: "Master 2",
    university: "ENS Yaoundé",
    slides: 24,
    gradient: "linear-gradient(135deg, #db2777 0%, #e11d48 50%, #dc2626 100%)",
    accent: "#e11d48",
    icon: "🎓",
  ...
  },
  {
    ...
    field: "Informatique & Sécurité",
    category: "informatique",
    level: "Master 2",
    university: "Université de Ngaoundéré",
    slides: 21,
    gradient: "linear-gradient(135deg, #1e293b 0%, #1e3a5f 50%, #1d4ed8 100%)",
    accent: "#1d4ed8",
    icon: "🔐",
  ...
  },
  {
    ...
    category: "management",
    level: "Licence 3",
    university: "IRIC Yaoundé",
    slides: 17,
    gradient: "linear-gradient(135deg, #7c3aed 0%, #a21caf 50%, #c026d3 100%)",
    accent: "#a21caf",
    icon: "👥",
  ...
  },
  {
    ...
    field: "Génie Électrique & Développement",
    category: "science",
    level: "Master 2",
    university: "École Polytechnique de Yaoundé",
    slides: 25,
    gradient: "linear-gradient(135deg, #16a34a 0%, #059669 50%, #0d9488 100%)",
    accent: "#059669",
    icon: "☀️",
...
function TiltCard({ work, onClick }: { work: PortfolioWork; onClick: () => void }) {
  ...
  const handleMouseMove = (e: MouseEvent<HTMLDivElement>) => {
    ...
    if (glowRef.current) {
      glowRef.current.style.background = `radial-gradient(circle at ${x}px ${y}px, rgba(255,255,255,0.18) 0%, transparent 65%)`
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Gradient-only decorative shapes */}
        {!hasImage && (
        ...
        <div ref={glowRef} className="absolute inset-0 transition-all duration-200 pointer-events-none" style={{ borderRadius: 'inherit' }} />

        {/* Hover overlay */}
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300" style={{ background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(2px)' }}>
          <span className="flex items-center gap-2 text-white text-sm font-semibold px-5 py-2.5 rounded-full" style={{ background: 'rgba(255,255,255,0.18)', border: '1px solid rgba(255,255,255,0.35)', backdropFilter: 'blur(8px)' }}>
        ...
        <div className="relative z-10 flex items-start justify-between p-5">
          ...
          <div className="flex items-center gap-2">
            {work.videoUrl && (
              <span className="text-white/90 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                <Film size={11} /> Vidéo
              </span>
            )}
            {hasImage && (
              <span className="text-white/90 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1" style={{ background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                <Image size={11} /> {work.images.length} slides
              </span>
            )}
            {!hasImage && (
              <span className="text-white/80 text-xs font-semibold px-3 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.15)', backdropFilter: 'blur(4px)', border: '1px solid rgba(255,255,255,0.2)' }}>
                {work.slides} slides
              </span>
            )}
          </div>
        ...
        <div className="relative z-10 p-5">
          ...
          <div className="text-white font-bold text-base leading-snug line-clamp-2" style={{ textShadow: '0 1px 8px rgba(0,0,0,0.5)' }}>
            {work.title}
          </div>
          <div className="flex items-center gap-2 mt-2.5">
            <span className="text-white/70 text-xs">{work.level}</span>
            <span className="w-1 h-1 rounded-full bg-white/40" />
            <span className="text-white/70 text-xs truncate">{work.university}</span>
          </div>
        </div>
      </div>

      {/* Tags strip */}
      <div className="bg-white flex items-center gap-2 px-4 py-3 flex-wrap">
        {work.tags.slice(0, 2).map(t => (
          <span key={t} className="text-xs font-medium px-2.5 py-1 rounded-full" style={{ background: '#f1f5f9', color: '#475569' }}>
            {t}
          </span>
        ))}
...
function SlideCarousel({ images, accent }: { images: string[]; accent: string }) {
  ...
  if (!images.length) return (
    <div className="flex flex-col items-center justify-center py-16 text-slate-400 gap-3" style={{ background: '#f8fafc', borderRadius: '1rem' }}>
  ...
  return (
    <div>
      ...
      <div className="relative rounded-2xl overflow-hidden" style={{ aspectRatio: '16/9', background: '#0f172a' }}>
        ...
        {/* Counter badge */}
        <div className="absolute top-3 right-3 text-xs font-bold text-white px-3 py-1.5 rounded-full" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(8px)' }}>
          {idx + 1} / {images.length}
        </div>
        ...
        {images.length > 1 && (
          <>
            <button
              onClick={prev}
              className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center text-white transition-all"
            ...
            <button
              onClick={next}
              className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full flex items-center justify-center text-white transition-all"
              style={{ background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(8px)', border: '1px solid rgba(255,255,255,0.2)' }}
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(0,0,0,0.7)'}
              onMouseLeave={e => e.currentTarget.style.background = 'rgba(0,0,0,0.45)'}
            >
        ...
        <div className="flex gap-2 mt-3 overflow-x-auto pb-1" style={{ scrollbarWidth: 'none' }}>
          {images.map((src, i) => (
            <button
              ...
              style={{
                width: 72, height: 45,
                outline: i === idx ? `2.5px solid ${accent}` : '2.5px solid transparent',
...
// ─── VIDEO PLAYER ─────────────────────────────────────────────
function VideoPlayer({ videoUrl }: { videoUrl: string; gradient: string }) {
  if (!videoUrl) return (
    <div className="flex flex-col items-center justify-center py-16 gap-4 rounded-2xl" style={{ background: '#f8fafc', border: '2px dashed #e2e8f0' }}>
      <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #e0e7ff, #ede9fe)' }}>
        <Film size={28} style={{ color: '#6366f1' }} />
      </div>
      <div className="text-center">
        <p className="text-slate-700 font-semibold text-sm mb-1">Aucune vidéo disponible</p>
        <p className="text-slate-400 text-xs">La vidéo de présentation n'a pas encore été ajoutée</p>
      </div>
    </div>
  )
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: work.gradient, padding: '1.5rem 1.75rem 1.25rem', minHeight: 110 }}
        >
          <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="relative z-10 flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-white font-bold text-base leading-snug max-w-md" style={{ textShadow: '0 2px 12px rgba(0,0,0,0.3)' }}>
                  {work.title}
                </div>
              </div>
            </div>
            <button
              onClick={onClose}
              className="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-white transition-all"
        ...
        {/* ── Tab bar ── */}
        <div className="flex-shrink-0 border-b border-slate-100" style={{ background: '#fff', padding: '0 1.5rem' }}>
          <div className="flex gap-1">
            {tabs.map(({ key, label, icon: Icon, badge }) => (
              <button
                ...
                style={{
                  color: activeTab === key ? work.accent : '#94a3b8',
              ...
              >
                ...
                {badge !== null && badge !== undefined && (
                  <span
                    ...
                    style={{ background: activeTab === key ? `${work.accent}18` : '#f1f5f9', color: activeTab === key ? work.accent : '#94a3b8', fontSize: 10 }}
                  >
                    {badge}
                  </span>
                )}
                {key === 'video' && hasVideo && (
                  <span className="w-1.5 h-1.5 rounded-full bg-red-400 absolute top-2.5 right-1" />
                )}
              </button>
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'thin', scrollbarColor: `${work.accent}30 transparent` }}>
          <div className="p-5">
            ...
            {activeTab === 'info' && (
              <div style={{ animation: 'fadeIn 0.2s ease' }}>
                ...
                <div className="grid grid-cols-3 gap-3 mb-5">
                  {[
                    { label: 'Niveau', value: work.level },
                    { label: 'Slides', value: `${work.slides}` },
                    { label: 'Établissement', value: work.university.split(' ').slice(0, 2).join(' ') },
                  ].map(({ label, value }) => (
                    <div key={label} className="text-center rounded-2xl py-3 px-2" style={{ background: '#f8fafc', border: '1px solid #e2e8f0' }}>
                      <div className="text-xs text-slate-400 mb-1">{label}</div>
                      <div className="text-xs font-bold text-slate-700 leading-tight">{value}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-5">
                  {work.tags.map(t => (
                    <span key={t} className="text-xs font-semibold px-3 py-1.5 rounded-full" style={{ background: `${work.accent}15`, color: work.accent }}>
                      {t}
                    </span>
                  ))}
                </div>
                ...
                {/* Highlights */}
                <div className="mb-5">
                  <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Ce que nous avons créé</div>
                  <ul className="space-y-2.5">
                    {work.highlights.map(h => (
                      <li key={h} className="flex items-start gap-3 text-sm text-slate-600">
                        <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: `${work.accent}20` }}>
                  ...
                  <div className="flex gap-2 mb-5">
                    {hasImages && (
                      <button
                        ...
                        onMouseEnter={e => e.currentTarget.style.background = `${work.accent}18`}
                ...
                <Link
                  ...
                  onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-1px)'; e.currentTarget.style.boxShadow = `0 12px 32px ${work.accent}50` }}
                  onMouseLeave={e => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.boxShadow = `0 8px 24px ${work.accent}40` }}
                >
                  <Zap size={15} />
                  Je veux le même pour moi
            ...
            {activeTab === 'video' && (
              <div style={{ animation: 'fadeIn 0.2s ease' }}>
                <VideoPlayer videoUrl={work.videoUrl} gradient={work.gradient} />
                <Link
                  to="/rendez-vous"
                  onClick={onClose}
                  className="w-full flex items-center justify-center gap-2 py-3.5 rounded-2xl text-white font-bold text-sm transition-all mt-5"
  ...
  return (
    <div className="font-body">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden" style={{ background: 'linear-gradient(160deg, #f8faff 0%, #eef2ff 40%, #f0fdf4 100%)' }}>
        <div className="absolute top-0 left-0 w-96 h-96 rounded-full pointer-events-none" style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', opacity: 0.15, filter: 'blur(80px)', transform: 'translate(-30%, -30%)' }} />
        <div className="absolute bottom-0 right-0 w-80 h-80 rounded-full pointer-events-none" style={{ background: 'linear-gradient(135deg, #06b6d4, #3b82f6)', opacity: 0.12, filter: 'blur(60px)', transform: 'translate(20%, 20%)' }} />
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-semibold mb-6" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366f1', border: '1px solid rgba(99,102,241,0.2)' }}>
            <Award size={13} />Nos réalisations
          </div>
          <h1 className="font-display font-bold text-slate-900 mb-5 leading-tight" style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)' }}>
          ...
          <p className="text-lg text-slate-500 max-w-xl mx-auto mb-10">
            Découvre les PowerPoints que nous avons créés pour d'autres étudiants. Clique sur une réalisation pour voir les slides et la vidéo.
          </p>
          <div className="inline-flex items-center gap-8 px-8 py-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.8)', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.9)', boxShadow: '0 8px 32px rgba(99,102,241,0.1)' }}>
            {[
      ...
      {/* ── FILTER TABS ──────────────────────────────────────── */}
      <section className="bg-white sticky top-16 lg:top-20 z-30 border-b border-slate-100" style={{ boxShadow: '0 4px 24px rgba(0,0,0,0.06)' }}>
      ...
      <section className="py-16" style={{ background: '#f8fafc' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="text-slate-400 text-sm mb-8">
            <span className="font-semibold text-slate-700">{filtered.length}</span> réalisation{filtered.length > 1 ? 's' : ''}
          </p>
      ...
      <section className="relative py-24 overflow-hidden" style={{ background: 'linear-gradient(135deg, #312e81 0%, #4f46e5 40%, #0891b2 100%)' }}>
        ...
        <div className="max-w-3xl mx-auto px-4 text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-semibold mb-6" style={{ background: 'rgba(255,255,255,0.15)', color: '#fff', border: '1px solid rgba(255,255,255,0.25)' }}>
            <Sparkles size={13} />Prêt à impressionner ton jury ?
          </div>
          <h2 className="font-display font-bold text-white mb-4" style={{ fontSize: 'clamp(2rem, 5vw, 3rem)' }}>
            Tu veux le même pour ta soutenance ?
          </h2>
          <p className="mb-10 text-lg" style={{ color: 'rgba(255,255,255,0.7)' }}>
            Prends rendez-vous et reçois ta présentation en <strong style={{ color: '#fff' }}>72h</strong>.
          </p>
          <Link
            to="/rendez-vous"
